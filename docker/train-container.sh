...
prepare_compose() {
  ...
  case "${TRAINING_TARGET}" in
    wsl)
      COMPOSE_FILE="${SRC_ROOT}/docker/compose.amd-wsl.yaml"
      ;;
    rocm)
      COMPOSE_FILE="${SRC_ROOT}/docker/compose.amd-rocm.yaml"
      ;;
  esac
}
...
usage() {
  cat <<'EOF'
Usage:
  src/docker/train-container.sh build
  src/docker/train-container.sh check
  src/docker/train-container.sh target
  src/docker/train-container.sh shell
  src/docker/train-container.sh install-frikbotnex
  src/docker/train-container.sh install-frikbotnex-train
  src/docker/train-container.sh build-worker
  src/docker/train-container.sh live-check
  src/docker/train-container.sh migrate-legacy
  src/docker/train-container.sh run -- <command...>
  src/docker/train-container.sh combat-bot-multi-verify {plan|report|check|collect|bc|eval-bc|ppo|eval}
  src/docker/train-container.sh combat-bot-multi {plan|report|check|collect|bc|eval-bc|ppo|eval}
  src/docker/train-container.sh combat-bot-<scenario>[-verify] {plan|report|check|collect|bc|eval-bc|ppo|eval}

Preferred workflow (new scripts):
  scripts/container.sh              # launch the container
  scripts/build-mod.sh              # build mods + workers (inside container)
  scripts/train.sh --bc             # one-time BC bootstrap (inside container)
  scripts/train.sh --ppo            # iterative PPO training (inside container)
...
case "${COMMAND}" in
  ...
  check)
    shift
    require_target_prereqs
    run_in_trainer 0 python -m quake_ai.utils.check_accelerator --device "${1:-$QUAKE_AI_DEVICE}" --fail-on-error
    ;;
  shell)
    shift
    require_target_prereqs
  ...
  install-frikbotnex)
    shift
    require_target_prereqs
    EXTRA_VOLUMES="--volume ${HOST_REPO_ROOT}/scripts:/scripts:ro" run_in_trainer 1 rw python /scripts/install_frikbotnex.py --asset-root /assets "$@"
    ;;
  install-frikbotnex-train)
    shift
    require_target_prereqs
    EXTRA_VOLUMES="--volume ${HOST_REPO_ROOT}/scripts:/scripts:ro" run_in_trainer 1 rw python /scripts/install_training_gamedir.py --asset-root /assets "$@"
    ;;
  build-worker)
    shift
    require_target_prereqs
  ...
  live-check)
    shift
    require_target_prereqs
    run_in_trainer 1 python -m quake_ai.rl.training --profile combat-bot-multi-verify --action check --device "${QUAKE_AI_DEVICE}" "$@"
    ;;
  migrate-legacy)
    shift
  ...
  run)
    ...
    run_in_trainer 0 "$@"
    ;;
  combat-bot-multi|combat-bot-multi-verify|combat-bot-*)
    shift
    require_target_prereqs
    action="${1:-bc}"
    shift || true
    run_in_trainer 1 python -m quake_ai.rl.training --profile "${COMMAND}" --action "${action}" --device "${QUAKE_AI_DEVICE}" "$@"
    ;;
  help|-h|--help)
    usage
    ;;

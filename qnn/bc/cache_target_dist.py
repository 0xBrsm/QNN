"""Populate the target_dist sidecar cache for an existing BC corpus.

Walks each shard in ``<cache_dir>/precomputed_train`` and
``<cache_dir>/precomputed_val``, runs the target labeler, and writes
``shard_NNNNNN_act_target_dist.npy`` (float16, ``(T_shard, 17)``) next
...
Use this when you have an existing native_v1 cache (no ``target_dist``
field in any shard's actions dict) and want to convert it to a cached
form without recollecting from demos.

Usage:
    python -m qnn.bc.cache_target_dist artifacts/collect/qwd
...
def _populate_one_shard(args: tuple) -> tuple[int, str, int]:
    """Worker: compute target_dist for one shard, write to disk.

    Returns (shard_idx, target_dist_fname, n_rows) so the caller can
    update the manifest. Skips if the shard already has target_dist.
    """
    split_dir_str, shard_idx, shard = args
    split_dir = Path(split_dir_str)

    if "target_dist" in shard.get("actions", {}):
        # Already cached — nothing to do for this shard.
        return shard_idx, shard["actions"]["target_dist"], 0
    ...
    # live train loader takes when target_dist isn't cached.
    ...
    fname = f"shard{shard_idx:06d}_act_target_dist.npy"
...
def _populate_split(split_dir: Path, n_workers: int) -> None:
    ...
    cached_already = sum(1 for s in shards if "target_dist" in s.get("actions", {}))
    pending = [(str(split_dir), i, s) for i, s in enumerate(shards)
               if "target_dist" not in s.get("actions", {})]
    if not pending:
    ...
    with ProcessPoolExecutor(max_workers=n_workers, mp_context=ctx) as ex:
        ...
        for fut in as_completed(futures):
            ...
            shards[shard_idx].setdefault("actions", {})["target_dist"] = fname
            completed += 1
            total_rows += n_rows
            if completed % 10 == 0 or completed == len(pending):
                print(f"    {completed}/{len(pending)} shards  ({total_rows:,} rows cached)")

    # Persist manifest with the new target_dist references.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Populate target_dist cache for an existing BC corpus.")
    parser.add_argument(
        "cache_dir",
        type=Path,

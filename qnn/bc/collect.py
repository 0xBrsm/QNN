...
#   offset 3  : op_input   u8   — strict per-axis OPERATIVENESS mask (press
...
#                                  Formerly the _pad byte — additive, the
#                                  struct stays 16 B so all other fields are
#                                  byte-identical.
...
_VALID_ACTION_FIELDS = frozenset({"move", "look", "attack", "weapon", "input_mask", "op_input", "target_probs"})
...
# Per-demo fire-script fingerprints (weapon-head.md §10-11): demo basename
# -> (weapon_script, weapon_release).  Loaded in main() before the pool
# forks; _collect_one_demo* stamps the pair into the collect op so the
# worker's de-scripted weapon intent label can suppress release-half
# alias dumps.  Generated by scripts/analysis/_weapon_script_scan.py
# --fingerprint-out from the raw corpus.
_WEAPON_SCRIPT_FP: dict[str, tuple[int, int]] = {}
...
def _collect_one_demo(proc: subprocess.Popen, demo_name: str,
    ...
    ws, wr = _WEAPON_SCRIPT_FP.get(os.path.basename(demo_name), (0, 0))
    op = {
        ...
        # Fire-script fingerprint for the de-scripted weapon intent label.
        "weapon_script": ws, "weapon_release": wr,
    }
...
def _compact_action_arrays(act_arrays: dict[str, np.ndarray]) -> None:
    ...
    if weapon is not None:
        ...
        # act_weapon is now the carry-forward + QC-validated-impulse label
        # (worker side, shared QOBS/LOBS): it reads weapon-select impulse
        # edges, never the laggy self_weapon_id stat, so the sub-50ms ABA
        # obs-echo flicker that the old min-dwell debounce removed can no
        # longer occur. No debounce.
...
def main() -> None:
    ...
    parser.add_argument(
        "--weapon-script-fingerprints",
        default="",
        help="Per-demo fire-script fingerprint JSON (weapon-head.md §10-11),"
             " generated by scripts/analysis/_weapon_script_scan.py"
             " --fingerprint-out.  Required for QWD collects — the worker's"
             " de-scripted weapon intent label needs each demo's scripted"
             " bit + release target to suppress +weapon alias release dumps."
             " Default: <corpus>/<demo-dir>_script_fingerprints.json next to"
             " the manifest.  Pass --no-weapon-script-fingerprints to opt"
             " out explicitly (all demos treated unscripted).",
    )
    parser.add_argument(
        "--no-weapon-script-fingerprints",
        action="store_true",
        help="Collect without a fire-script fingerprint file: every demo is"
             " treated as unscripted (no release-dump suppression). The"
             " intent-label state machine still applies. Only for corpora"
             " known to be script-free.",
    )
    ...
    # Fire-script fingerprints (weapon-head.md §10-11).  QWD-only: the
    # de-scripted intent label reads usercmds, which .mvd demos lack.
    if demo_type == "qwd" and not args.no_weapon_script_fingerprints:
        fp_path = (Path(args.weapon_script_fingerprints)
                   if args.weapon_script_fingerprints
                   else demo_dir.parent / f"{demo_dir.name}_script_fingerprints.json")
        if not fp_path.exists():
            raise SystemExit(
                f"weapon-script fingerprints not found: {fp_path}\n"
                "Generate with: scripts/analysis/_weapon_script_scan.py "
                f"--corpus-dir {demo_dir} --fingerprint-out {fp_path}\n"
                "(or pass --no-weapon-script-fingerprints for a corpus known "
                "to be script-free)")
        fp_map = json.loads(fp_path.read_text())
        _WEAPON_SCRIPT_FP.update({
            name: (1, int(v["release_target"]))
            for name, v in fp_map.items()
            if v.get("scripted") and v.get("release_target")
        })
        print(f"Fire-script fingerprints: {len(_WEAPON_SCRIPT_FP)} scripted "
              f"demos from {fp_path}")
        # Pin alongside the cache so provenance travels with the shards.
        output.mkdir(parents=True, exist_ok=True)
        (output / "weapon_script_fingerprints.json").write_text(
            fp_path.read_text())

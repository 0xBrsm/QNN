...
def resolve_asset_root(explicit: str | None) -> Path:
    if not explicit:
...
def validate_native_mod_assets(asset_root: Path, native_args: Sequence[str] | None) -> None:
...
def resolve_demo_dir_from_run(run_cfg: dict[str, Any], explicit: str | None = None) -> Path | None:
    if explicit:
        candidate = Path(explicit)
    ...
    machine = run_cfg.get("machine")
    if not isinstance(machine, Mapping):
        return None
    demo_dir_str = machine.get("demo_dir", "")
    if not demo_dir_str:
        return None
...
def write_run_plan(
    ...
    payload: dict[str, Any] = {
        ...
        "scenario_note": scenario_note(run_cfg) if "scenario" in run_cfg else "",

"""Stacked OFAT-winner attack head.

Combines the three winning OFAT factors from the attack-prior sweep on
top of :class:`LookStyleAttackHead`'s geometric prior:

  1. ``engagement_ema`` scalar     (cf. :class:`EngagedLookStyleAttackHead`)
  2. per-weapon embedding          (cf. :class:`WeaponEmbedAttackHead`)
  3. soft-pooled target geometry   (cf. :class:`GeomAttackHead`)
     — ``dist_norm``, ``radial_norm``, ``tang_norm``
...
cast back to ``inp.features.dtype`` before concatenation.
...
class EngagedGeomWeaponEmbedAttackHead(nn.Module):
    """LookStyle attack head stacked with engagement_ema + per-weapon embed + soft geom scalars."""
    ...
    def __init__(
        ...
        *,
        ...
        mlp_in_dim = in_dim + 1 + self.weapon_embed_dim + 3
    ...
    def forward(self, inp: AttackHeadInput) -> AttackHeadOutput:
        assert inp.target_logits is not None, "EngagedGeomWeaponEmbedAttackHead needs target_logits"
        assert inp.entity_scalars is not None, "EngagedGeomWeaponEmbedAttackHead needs entity_scalars"
        assert inp.actor_mask is not None, "EngagedGeomWeaponEmbedAttackHead needs actor_mask"
        assert inp.weapon_id is not None, "EngagedGeomWeaponEmbedAttackHead needs weapon_id"

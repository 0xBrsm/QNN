"""Polar (magnitude × direction) look head — the canonical look head.

Promoted to core in a28 (it had lived in the a24 bench namespace since it
won that generation's ablation). Decomposes the view-relative turn-delta
into:

    mag ∈ {hold} ∪ {N_MAG foveated turn magnitudes}   (categorical)
    dir ∈ {N_DIR uniform directions in [0, 2π)}        (categorical)

Why polar (see qnn.model.look_bins): "hold" (don't turn) is a SINGLE
protected bin rather than the coincidence of two center bins, and a
flick's yaw/pitch are one direction (their correlation is represented,
not factorized away). This is what lets SAMPLING reproduce the human turn
distribution — the dominant hold mode and directional flicks both — which
the decoded mean / per-axis bins cannot.

Input is the shared feature cat, prefix-sliced to the head's declared
graph inputs — no prior, no pointer, no position, and (a28) no
weapon_ctx: what the graph declares is everything this head sees.

The head emits the two logit tensors (consumed by ``look_loss`` as
hierarchical cross-entropy) plus a deterministic ``look_predict`` (argmax
reconstruction) for diagnostics. ACTING is done by
``qnn.model.look_bins.polar_sample`` (sample → 3D unit vector for the
engine); PPO log-probs by ``polar_log_prob``.
"""
...
class LookHeadOutput:
    look_predict: torch.Tensor      # (B*, 3) argmax reconstruction (diagnostics)
    look_mag_logits: torch.Tensor   # (B*, N_MAG+1)  [0]=hold
    look_dir_logits: torch.Tensor   # (B*, N_DIR)


class PurePolarLookHead(nn.Module):
    """Look head: MLP(features) → magnitude + direction categoricals."""
    ...
    def forward(self, features: torch.Tensor) -> LookHeadOutput:
        h = self.mlp(features[..., : self.in_dim])
    ...
    def look_loss(self, logits, look_label, valid, compute_metrics):
        ...
        if compute_metrics:
            ...
            # histogram the binned head used, so look_dll stays on the same
            # tangent-density scale across head families.
            with torch.no_grad():
                from qnn.model.look_bins import (
                    POLAR_LOG_CELL_AREA, N_BINS, bin_targets,
                )
                V = int(mag.shape[0])
                ...
                # Human binned tangent hist — same format as the binned head used.
...
def _build_look_polar(head, dims, d_model):
    ...
    if "target.feat" not in head.inputs:
        in_dim -= d_model
    return PurePolarLookHead(in_dim, head.d_hidden, head.activation)

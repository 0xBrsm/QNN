...
class _ActorCriticNet(nn.Module):
    def __init__(self, obs_dim: int, trunk_hidden: int, gru_hidden: int, use_gru: bool) -> None:
        super().__init__()
    ...
    def forward(
        ...
        *,
        ...
        input_is_sequence = obs.ndim == 3
        if input_is_sequence:
            seq_len, batch_size, obs_dim = obs.shape
            flat_obs = obs.reshape(seq_len * batch_size, obs_dim)
        ...
        if self.use_gru:
            ...
            next_hidden = torch.zeros((batch_size, 0), dtype=obs.dtype, device=obs.device)
...
class MLPGRUPolicy:
    ...
    def _maybe_pad_obs_batch(self, obs_tensor: torch.Tensor, hidden_tensor: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, int]:
        if self.use_gru or obs_tensor.ndim != 2:
            return obs_tensor, hidden_tensor, 0
        ...
        padded_obs = F.pad(obs_tensor, (0, 0, 0, pad_rows))
    ...
    def _forward_tensors(
        ...
        *,
        ...
        if obs_tensor.ndim == 2:
            ...
            padded_obs, padded_hidden, pad_rows = self._maybe_pad_obs_batch(obs_tensor, hidden_t)
        ...
        seq_len, batch_size, _ = obs_tensor.shape
        ...
        return self.model(obs_tensor, hidden=hidden_t, masks=mask_t)
    ...
    def ppo_policy_step(
        ...
        *,
        ...
        for head in ACTION_HEADS:
            target = self._flatten_targets(self._tensor(actions[head], dtype=torch.long))
            chosen_log_probs, _, _ = self._log_prob_entropy_from_logits(self._flatten_logits(logits[head]), target)
    ...
    @classmethod
    def _copy_compatible_parameters(cls, source: "MLPGRUPolicy", target: "MLPGRUPolicy") -> None:
        ...
        for key, target_value in target_state.items():
            ...
            if source_value.shape == target_value.shape:
                ...
                continue
            slices = tuple(slice(0, min(int(src), int(dst))) for src, dst in zip(source_value.shape, target_value.shape))
    ...
    def load(cls, path: str | Path, device: str = "auto") -> "MLPGRUPolicy":
        ...
        try:
            ...
            if isinstance(payload, dict) and "state_dict" in payload and "meta" in payload:
                ...
                model.model.load_state_dict(payload["state_dict"], strict=False)
    ...
    def load_for_finetune(
        ...
        *,
        ...
        cls._copy_compatible_parameters(loaded, target)
        return target

...
Usage:
    python -m quake_ai.rl.quickeval runs/tracking_arena_v6_20260327 \
        --checkpoint best/best_000000336_2752512_reward_12.921.pth

    # Dump per-tick metrics:
    python -m quake_ai.rl.quickeval runs/... --checkpoint ... --dump-ticks /tmp/ticks
"""
...
from quake_ai.rl.evaluation import EvalConfig, run_evaluation, _load_checkpoint
...
def main() -> None:
    ...
    if args.dump_ticks is not None:
        ...
        config = EvalConfig(
            ...
            native_env={"QUAKE_BASEDIR": str(_resolve_asset_root(_require_string(machine, "asset_root", "machine.json")))},
            native_args=list(scenario.get("native_args", [])),

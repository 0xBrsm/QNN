...
from quake_ai.rl.run_config import load_run_config, run_output_dirs
from quake_ai.rl.reporting import write_run_report
from quake_ai.utils.io import read_json, trusted_torch_load, write_json
...
def build_runner_context(run_dir: Path) -> RunnerContext:
    ...
    resolved_asset_root = _resolve_asset_root(require_cfg_string(machine, "asset_root", "machine.json"))
    worker_path = Path(require_cfg_string(machine, "worker_binary", "machine.json"))
    ...
    plan_path = write_run_plan(
        ...
        runtime,
        plan,
        resolve_demo_dir_from_run(run_cfg) if mode == "bc" else None,
        resolved_asset_root,
    )
...
def ensure_demo_worker(demo_worker_binary: Path, rebuild: bool) -> Path:
...
def materialize_child_run(
    ...
    *,
    ...
    config_payloads = {
        "trainer": _merged_payload(require_cfg_mapping(parent_run_cfg, "trainer", "run config"), trainer_overrides),

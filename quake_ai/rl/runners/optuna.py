...
from quake_ai.rl.runners.ppo import run_pipeline as run_ppo_pipeline
...
def _evaluate_trial(trial_ctx: RunnerContext, checkpoint_path: Path, output_root: Path) -> tuple[float, dict[str, float]]:
    ...
    trainer = require_cfg_mapping(trial_ctx.run_cfg, "trainer", "run config")
...
def run(ctx: RunnerContext) -> dict[str, Any]:
    ...
    def _reload_run_cfg() -> dict[str, Any]:
        ...
        from quake_ai.rl.run_config import load_run_config
